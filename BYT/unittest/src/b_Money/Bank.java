...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		// found by testOpenAccount, exception when trying to get nonexistent account
		if (accountlist.containsKey(accountid)) {
			throw new AccountExistsException();
		} else {
			// invalid creation logic
			// put new item instead of getting it
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		// found by testDeposit, exception for valid account
		// invalid condition
		if (accountlist.containsKey(accountid)) {
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		// found by testWithdraw, no money subtracted from balance
		if (!accountlist.containsKey(accountid)) {
			throw new AccountDoesNotExistException();
		}
		else {
			Account account = accountlist.get(accountid);
			account.withdraw(money); // wrong method call
		}
	}
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		// found by testTransfer, transfers money to itself
		transfer(fromaccount, this, toaccount, amount); // invalid toaccount
	}
	...
	public void addTimedPayment(String accountid, String payid, Integer interval, Integer next, Money amount, Bank tobank, String toaccount) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			throw new AccountDoesNotExistException(); // check if account exists
		} else {
			Account account = accountlist.get(accountid);
	 ...
	 * @return account associated with accountid 
	 * @throws AccountDoesNotExistException If the account does not exist
	 */
